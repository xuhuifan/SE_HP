...
for ite = 1:IterationTime
    ...
    ss = time()
    sample_b_u!(hdbn, C_list, Lambdas, alpha_v, tau_v, b_ij, u_ij, sender_receiver_num, a_H)
    ...
    sample_Lambda!(hdbn, C_list, b_ij, u_ij, Lambdas, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list, c₀, r₀, rₖ, η, ξ, a_H)
    # println("sample_Lambda finished.")

    sample_C_list!(hdbn, C_list, pis_list, M_seq, b_ij, u_ij, Lambdas, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list, a_H)
    ...
    sample_K!(hdbn, C_list, pis_list, M_seq, b_ij, u_ij, Lambdas, t_i_s_list, poisson_i_value, a_H)
    # println("sample_K finished.")

    sample_poisson_i_val!(t_i_s_list, poisson_i_value, hdbn.dataNum)

    sample_change_point!(hdbn, C_list, pis_list, b_ij, u_ij, Lambdas, t_i_s_list, a_H)
    ...
    ll_val_part_1, ll_val_part_2, ll_val_part_3, ll_val_part_4 = ll_cal!(hdbn, b_ij, alpha_v, tau_v, u_ij, Lambdas, C_list, sender_receiver_num, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list, a_H)
...
savefig(join(["full_plot_", Name_seq[data_i], ".pdf"])) 
