...
function sample_b_u!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, Lambdas ::Array{Float64,2}, alpha_v ::Array{Float64,1}, tau_v ::Array{Float64,1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, sender_receiver_num ::Array{Int64, 2}, a_H::Array{Float64, 1})
    ...
    for tt = 1:hdbn.EdgeNum
        ...
        for k1 = 1:hdbn.KK
            for k2 = 1:hdbn.KK
                rate_vector[k1+ (k2-1)*(hdbn.KK)] = C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][k1]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][k2]*Lambdas[k1,k2]/(a_H[1]^2)
            end
        end
...
function sample_C_list!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, M_seq ::Array{Float64, 1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, Lambdas ::Array{Float64,2}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64,1})
    ...
    for tt = 1:length(flattern_t_i_s_nodes)
        ...
        for k1 = 1:hdbn.KK
            ...
            a1 = -sum(integral_val_tt.*(Lambdas[:, k1]+Lambdas[k1, :]))/(a_H[1]^2)
...
function sample_M_seq!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, M_seq ::Array{Float64, 1}, a_H ::Array{Float64, 1}, b_H ::Array{Float64, 1})
    ...
    if a_H_new>0
        ...
        for tt = 1:hdbn.EdgeNum
            if b_ij[tt]==0
                current_ll += log(C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][u_ij[tt, 1]]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*Lambdas[u_ij[tt, 1], u_ij[tt, 2]]/(a_H[1]^2))
                proposal_ll += log(C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][u_ij[tt, 1]]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*Lambdas[u_ij[tt, 1], u_ij[tt, 2]]/(a_H_new^2))
            end
        end
    
        areas = cal_area(hdbn.KK, hdbn.dataNum, C_list, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list)
    
        current_ll -= sum(areas.*Lambdas./(a_H[1]^2))
        proposal_ll -= sum(areas.*Lambdas./(a_H_new^2))
...
function sample_K!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, M_seq ::Array{Float64, 1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, Lambdas ::Array{Float64,2}, t_i_s_list ::Array{Array{Float64,N} where N,1}, poisson_i_value ::Array{Float64, 1}, a_H::Array{Float64, 1})
    for ii = 1:hdbn.dataNum
        if rand()<0.5 # choose to propose a new change change point
            ...
            for k1 = 1:hdbn.KK
                for k2 = 1:hdbn.KK
                    current_ll -= C_list[ii][current_index][k1]*integral_val_tt[k2]*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                    proposal_ll -= C_ii[k1]*integral_val_tt[k2]*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                end
            end
            for k1 = 1:hdbn.KK
            ...
            if length(t_i_s_list[ii])>0
                ...
                for k1 = 1:hdbn.KK
                    for k2 = 1:hdbn.KK
                        current_ll -= C_list[ii][current_index+1][k1]*integral_val_tt[k2]*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                        proposal_ll -= C_list[ii][current_index][k1]*integral_val_tt[k2]*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                    end
                end
                for k1 = 1:hdbn.KK
...
function sample_change_point!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, Lambdas ::Array{Float64,2}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64, 1})
    for ii = 1:hdbn.dataNum
...
########################
                    ...
                    for k1 = 1:hdbn.KK
                        for k2 = 1:hdbn.KK
                            current_ll -= C_list[ii][jj][k1]*(diff_integral[k2])*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                            proposal_ll -= C_list[ii][jj+1][k1]*(diff_integral[k2])*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
...
########################
                    ...
                    for k1 = 1:hdbn.KK
                        for k2 = 1:hdbn.KK
                            current_ll -= C_list[ii][jj+1][k1]*(diff_integral[k2])*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
                            proposal_ll -= C_list[ii][jj][k1]*(diff_integral[k2])*(Lambdas[k1,k2]+Lambdas[k2,k1])/(a_H[1]^2)
...
function ll_cal!(hdbn ::HDBN, b_ij ::Array{Int64,1}, alpha_v ::Array{Float64,1}, tau_v ::Array{Float64,1}, u_ij ::Array{Int64,2}, Lambdas ::Array{Float64,2}, C_list ::Array{Array{Array{Float64,1},1},1}, sender_receiver_num::Array{Int64, 2}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64, 1})
    ...
    for tt = 1:hdbn.EdgeNum
        if b_ij[tt]==0
            ...
            C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*Lambdas[u_ij[tt, 1], u_ij[tt, 2]]/(a_H[1]^2))
    ...
    ll_val_part_3 -= sum(areas.*Lambdas./(a_H[1]^2))
...
function calculate_AUC(test_time_range, dataNum, KK, send_node_test, receive_node_test, eventtime_test, send_node, receive_node, eventtime, C_list_end, Lambdas, alpha_v, tau_v, a_H)
            ...
            for ii = 1:dataNum
                for jj = 1:dataNum
                    if ii!= jj
                        ...
                        for k1 = 1:KK
                            for k2 = 1:KK
                                rate += C_list_end[ii, k1]*Lambdas[k1,k2]*C_list_end[jj, k2]/(a_H[1]^2)
                            end
                        end
