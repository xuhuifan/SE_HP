...
function model_ini(send_node, receive_node, eventtime, dataNum, EdgeNum, KK, unique_send_connect, unique_receive_connect)
    ...
    a_alpha = 1.
    b_alpha = 1.
    log_α = log(rand(Gamma(a_alpha, 1.))/b_alpha)
    ...
    η = [1.]
    ξ = [1.]
    log_Λ = zeros(Float64, KK, KK)
    for k1 = 1:KK
        for k2 = 1:KK
            # if k1==k2
            #     log_Λ[k1, k2] = log(rand(Gamma(ξ[1]*rₖ[k1], 1.))/η[1])
            # else
            #     log_Λ[k1, k2] = log(rand(Gamma(rₖ[k1]*rₖ[k2], 1.))/η[1])
            # end
            log_Λ[k1, k2] = log(rand(Gamma(1/KK, 1.))/η[1])
        end
    end
    ...
    a_beta_12 = [1.0]
    b_beta_12 = [1.0]
    log_β = zeros(Float64, length(unique_send_connect))
    for tt = 1:length(unique_send_connect)
        if unique_send_connect[tt]==unique_receive_connect[tt]
            log_β[tt] = log(rand(Gamma(a_beta_11[1], 1.))/b_beta_11[1])
        else
            log_β[tt] = log(rand(Gamma(a_beta_12[1], 1.))/b_beta_12[1])
        end
    end
    ...
    for tt = 1:length(flattern_t_i_s_nodes)
        ...
        for ii_index = 1:length(sending_nodes_tt)
            ...
            concentration_parameter .+= exp(log_β[vv]).*pis_list[sending_nodes_tt[ii_index]][end]
        end
        for kk = 1:KK
    ...
    return log_τ, TT, a_M, b_M, M_seq, a_alpha, b_alpha, log_α, log_Λ, log_β,
...
function back_propagate!(hdbn::HDBN, pis_list ::Array{Array{Array{Float64,1},1},1}, C_list ::Array{Array{Array{Float64,1},1},1}, log_β ::Array{Float64,1}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_beta_11::Array{Float64,1}, b_beta_11::Array{Float64,1}, a_beta_12::Array{Float64,1}, b_beta_12::Array{Float64,1}, start_points::Float64, end_points::Float64)
    ...
    pseudo_start_point = maximum([0., start_points-(end_points-start_points)/2.])
    pseudo_end_point = minimum([hdbn.TT, end_points+(end_points-start_points)/2.])
    ...
    for tt = length(flattern_t_i_s_nodes):-1:1
        ...
        if (pseudo_start_point<current_change_point)&(pseudo_end_point>current_change_point)
            ...
            for ii_index = 1:length(sending_nodes_tt)
                ...
                psi_same_layer[ii_index, :] = exp(log_β[vv_seq[ii_index]]).*pis_list[sending_nodes_tt[ii_index]][rank_seq[ii_index]]
            end
            # rank_total[tt] = rank_seq
            psi_sum = reshape(sum(psi_same_layer[1:length(sending_nodes_tt), :], dims = 1), hdbn.KK)

            if (start_points<current_change_point)&(end_points>current_change_point)
...
function sample_b_u!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, log_Λ ::Array{Float64,2}, log_α ::Array{Float64,1}, log_τ ::Array{Float64,1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, sender_receiver_num ::Array{Int64, 2}, a_H::Array{Float64, 1}, subset_index::Array{Int64, 1})

    rate_vector = Array{Float64}(undef, hdbn.max_mutually_len+hdbn.KK^2)
    for tt = 1:hdbn.EdgeNum

        if tt in subset_index
            for k1 = 1:hdbn.KK
                for k2 = 1:hdbn.KK
                    rate_vector[k1+ (k2-1)*(hdbn.KK)] = C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][k1]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][k2]*exp(log_Λ[k1,k2])/(a_H[1]^2)
                end
            end
            for jj =1:hdbn.mutually_len[tt]
                rate_vector[jj+hdbn.KK^2] = exp(log_α[1])*exp(-exp(log_τ[1])*(hdbn.eventtime[tt]-hdbn.eventtime[hdbn.mutually_exciting_pair[tt][jj]]))
            end
...
function update_beta!(hdbn ::HDBN, pis_list ::Array{Array{Array{Float64,1},1},1}, log_β ::Array{Float64,1}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, start_points::Float64, end_points::Float64, p_β::Array{Float64,1}, noise_h::Float64, A::Float64, h::Float64, ϕ::Array{Float64, 1})
    ∇_β = zeros(Float64, length(log_β))
    psi_same_layer = zeros(Float64, hdbn.max_receive_len+1, hdbn.KK)
    ...
    for tt = 1:length(flattern_t_i_s_nodes)
        ...
        if (start_points<current_change_point)&(end_points>current_change_point)
            ...
            for ii_index = 1:length(sending_nodes_tt)
                ∇_β[vv_seq[ii_index]] += digamma(sum(psi_sum))
                for kk = 1:hdbn.KK
                    π_ik = pis_list[flattern_t_i_s_nodes[tt]][flattern_t_i_s_nodes_rank[tt]][kk]
                    ∇_β[vv_seq[ii_index]] -= π_ik*digamma(psi_sum[kk])
                    ∇_β[vv_seq[ii_index]] += π_ik*log(pis_list[flattern_t_i_s_nodes[tt]][flattern_t_i_s_nodes_rank[tt]+1][kk])
                end
            end
    ...
    ∇_β = ∇_β.*hdbn.TT./(end_points-start_points) .- 1.0

    ∇_β = -∇_β.*exp.(log_β)
    p_β = p_β .- ϕ[1].*p_β.*h .- ∇_β.*h .+ noise_h*sqrt(2*A)
...
function update_Lambda!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, log_Λ ::Array{Float64,2}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, c₀ ::Array{Float64, 1}, r₀ ::Array{Float64, 1}, rₖ ::Array{Float64, 1}, η ::Array{Float64, 1}, ξ ::Array{Float64, 1}, a_H::Array{Float64, 1}, start_points::Float64, end_points::Float64, subset_index::Array{Int64,1}, p_Λ::Array{Float64,2}, noise_h::Float64, A::Float64, h::Float64, ϕ::Array{Float64, 1})
    areas_sub = zeros(Float64, hdbn.KK, hdbn.KK)
    ...
    for tt = 1:(length(flattern_t_i_s_nodes)-1)
        ...
        t_i_1 = t_i_s_list[flattern_t_i_s_nodes[tt]][flattern_t_i_s_nodes_rank[tt]]
        t_i_2 = t_i_s_list[flattern_t_i_s_nodes[tt+1]][flattern_t_i_s_nodes_rank[tt+1]]

        if (end_points > t_i_1)&(start_points<t_i_2)
            if (start_points<t_i_1)&(end_points>t_i_2)
                areas_sub .+= ((reshape(sum(C_ii, dims = 1), hdbn.KK, 1)*reshape(sum(C_ii, dims = 1), 1, hdbn.KK) .- C_ii'*C_ii).*(t_i_2-t_i_1))
            elseif (start_points<t_i_1)&(end_points<t_i_2)
                areas_sub .+= ((reshape(sum(C_ii, dims = 1), hdbn.KK, 1)*reshape(sum(C_ii, dims = 1), 1, hdbn.KK) .- C_ii'*C_ii).*(end_points-t_i_1))
            elseif (start_points>t_i_1)&(end_points>t_i_2)
                areas_sub .+= ((reshape(sum(C_ii, dims = 1), hdbn.KK, 1)*reshape(sum(C_ii, dims = 1), 1, hdbn.KK) .- C_ii'*C_ii).*(t_i_2-start_points))
            end
        end

        if (start_points<t_i_1)&(end_points>t_i_1)
            areas_sub .+= ((reshape(sum(C_ii, dims = 1), hdbn.KK, 1)*reshape(sum(C_ii, dims = 1), 1, hdbn.KK) .- C_ii'*C_ii).*(t_i_2-t_i_2))
        end
    end

    Xₖ = zeros(Int64, hdbn.KK, hdbn.KK)
    select_u_ij = u_ij[subset_index, :]
    judge_b_0_u_ij = select_u_ij[b_ij[subset_index].==0, :]
    for k1 = 1:hdbn.KK
        for k2 = 1:hdbn.KK
            # println("a ")
            (judge_b_0_u_ij[:,1].==k1)
            # println("b ")
            (judge_b_0_u_ij[:,1].==k1).& (judge_b_0_u_ij[:,2].==k2)
            # println("c ")
    ...
    ∇_Λ = zeros(Float64,hdbn.KK, hdbn.KK)            
    for k1 = 1:(hdbn.KK)
        for k2 = 1:(hdbn.KK)
            ∇_Λ[k1,k2] -= areas_sub[k1,k2]/(a_H[1]^2)*hdbn.TT/(end_points-start_points)
            ∇_Λ[k1,k2] += Xₖ[k1,k2]/(exp(log_Λ[k1,k2]))*hdbn.TT/(end_points-start_points)
            ∇_Λ[k1,k2] -= η[1]
            if k1 == k2
                ∇_Λ[k1,k2] += (rₖ[k1]*ξ[1]-1)/exp(log_Λ[k1,k2])
            else
                ∇_Λ[k1,k2] += (rₖ[k1]*rₖ[k2]-1)/exp(log_Λ[k1,k2])
            end
        end
    end

    ∇_Λ = -∇_Λ.*exp.(log_Λ)
    p_Λ = p_Λ .- ϕ[1].*p_Λ.*h .- ∇_Λ.*h .+ noise_h*sqrt(2*A)
    # println("before log_Λ is: ", log_Λ)
    # println()
    # println()
    log_Λ = log_Λ .+ p_Λ.*h
    # println("after log_Λ is: ", log_Λ)
    # println()
    # println()
    # Lambdas = exp(log_Λ)
    # Lambdas = Lambdas .+ p_Λ.*h

    # areas = cal_area(hdbn.KK, hdbn.dataNum, C_list, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list)./(a_H[1]^2)
...
function sample_C_list!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, M_seq ::Array{Float64, 1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, log_Λ ::Array{Float64,2}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64,1}, start_points::Float64, end_points::Float64)

    CanNum = 1000
    log_proportions = Array{Float64}(undef, CanNum)
    proportions = Array{Float64}(undef, CanNum)
    vv = zeros(Float64, hdbn.KK)

    for tt = 1:length(flattern_t_i_s_nodes)
        current_time = t_i_s_list[flattern_t_i_s_nodes[tt]][flattern_t_i_s_nodes_rank[tt]]
        if current_time>end_points
            break
        end
        if (current_time>start_points)&(current_time<end_points)
            ...
            for k1 = 1:hdbn.KK
                ...
                for k2 = 1:hdbn.KK
                    vv[k2] = exp(log_Λ[k2,k1])+exp(log_Λ[k1,k2])
                end
                a1 = -sum(integral_val_tt.*vv)/(a_H[1]^2)
    ...
    for jj = 1:hdbn.dataNum
        ...
        if end_points>s_time  # more considerations are needed.
...
function sample_M_seq!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, log_Λ ::Array{Float64,2}, M_seq ::Array{Float64, 1}, a_H ::Array{Float64, 1}, b_H ::Array{Float64, 1})
    ...
    if a_H_new>0
        ...
        for tt = 1:hdbn.EdgeNum
            if b_ij[tt]==0
                current_ll += log(C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][u_ij[tt, 1]]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*exp(log_Λ[u_ij[tt, 1], u_ij[tt, 2]])/(a_H[1]^2))
                proposal_ll += log(C_list[hdbn.send_node[tt]][sender_receiver_num[tt, 1]][u_ij[tt, 1]]*C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*exp(log_Λ[u_ij[tt, 1], u_ij[tt, 2]])/(a_H_new^2))
            end
        end
    
        areas = cal_area(hdbn.KK, hdbn.dataNum, C_list, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list)
    
        current_ll -= sum(areas.*exp.(log_Λ)./(a_H[1]^2))
        proposal_ll -= sum(areas.*exp.(log_Λ)./(a_H_new^2))

        for ii = 1:dataNum
            proposal_ll += (a_H_new*log(b_H[1]*M_seq[ii])-loggamma(a_H_new))
            current_ll += (a_H[1]*log(b_H[1]*M_seq[ii])-loggamma(a_H[1]))
        end
        proposal_ll += 99.0*log(a_H_new)-a_H_new
        current_ll += 99.0*log(a_H[1])-a_H[1]
...
function update_α_τ!(hdbn::HDBN, b_ij::Array{Int64,1}, log_τ::Array{Float64,1}, log_α::Array{Float64,1}, start_points::Float64, end_points::Float64, subset_index::Array{Int64,1}, p_α::Array{Float64, 1}, p_τ::Array{Float64, 1}, noise_h::Float64, A::Float64, h::Float64, ϕ::Array{Float64, 1})
    # sum(b_ij[subset_index]>0)
    sum_v = 0.
    for subset_i in subset_index
        if b_ij[subset_i]>0
            sum_v += 1.0
        end
    end
    ∇_α  = (hdbn.TT/(end_points-start_points)*(sum((1 .- exp.(-exp(log_τ[1]).*(hdbn.TT .- hdbn.eventtime[subset_index])))./exp(log_τ[1]))+sum_v/exp(log_α[1]))-1)
    ∇_α = -∇_α*exp(log_α[1])

    p_α[1] = p_α[1] .- ϕ[1].*p_α[1].*h .- ∇_α.*h + noise_h*sqrt(2*A)
    log_α[1] = log_α[1] .+ p_α[1].*h

    ∇_τ = 0.
    for tt = 1:hdbn.EdgeNum
        if (hdbn.eventtime[tt]>start_points)&(hdbn.eventtime[tt]<end_points)
            ∇_τ += exp(log_α[1])/(exp(log_τ[1])^2)*(1-exp(-exp(log_τ[1])*(hdbn.TT-hdbn.eventtime[tt])))
            ∇_τ -= exp(log_α[1])/(exp(log_τ[1]))*(exp(-exp(log_τ[1])*(hdbn.TT-hdbn.eventtime[tt])))*(hdbn.TT-hdbn.eventtime[tt])
            if b_ij[tt]>0
                ∇_τ -= (hdbn.eventtime[tt]-hdbn.eventtime[hdbn.mutually_exciting_pair[tt][b_ij[tt]]])
            end
        end
    end
    ∇_τ = ∇_τ*hdbn.TT/(end_points-start_points)-1.0

    ∇_τ = -∇_τ*exp(log_τ[1])

    p_τ[1] = p_τ[1] .- ϕ[1].*p_τ[1].*h .- ∇_τ.*h + noise_h*sqrt(2*A)

    log_τ[1] = log_τ[1] .+ p_τ[1].*h

end

# function sample_alpha!(hdbn ::HDBN, b_ij ::Array{Int64,1}, log_τ ::Array{Float64,1}, log_α ::Array{Float64,1}, a_alpha ::Float64, b_alpha::Float64)
#     h_tau = 0
#     b_ij_over_1_sum = 0
#     for tt = 1:hdbn.EdgeNum
#         h_tau += (1-exp(-log_τ[1]*(hdbn.TT-hdbn.eventtime[tt])))
#         if (b_ij[tt]>0)
#             b_ij_over_1_sum += 1
#         end
#     end
...
#     exp(log_α[1]) = rand(Gamma(a_alpha+b_ij_over_1_sum, 1.))/(b_alpha+h_tau)
...
# function sample_tau!(hdbn ::HDBN, log_τ ::Array{Float64,1}, b_ij ::Array{Int64,1}, log_α ::Array{Float64,1})
#     a_tau = 0.1
#     b_tau = 0.1

#     new_log_τ = log_τ[1] + 0.1*randn()
#     if new_log_τ>0
#         ll_old = 0
#         ll_new = 0
...
#         for tt = 1:hdbn.EdgeNum
#             if b_ij[tt]>0
#                 ll_old -= log_τ[1]*(hdbn.eventtime[tt]-hdbn.eventtime[hdbn.mutually_exciting_pair[tt][b_ij[tt]]])
#                 ll_new -= new_log_τ*(hdbn.eventtime[tt]-hdbn.eventtime[hdbn.mutually_exciting_pair[tt][b_ij[tt]]])

#             end
#             ll_old -= exp(log_α[1])/log_τ[1]*(1-exp(-log_τ[1]*(hdbn.TT-hdbn.eventtime[tt])))
#             ll_new -= exp(log_α[1])/new_log_τ*(1-exp(-new_log_τ*(hdbn.TT-hdbn.eventtime[tt])))
#         end

#         ll_old += logpdf(Gamma(a_tau, 1/b_tau), log_τ[1])
#         ll_new += logpdf(Gamma(a_tau, 1/b_tau), new_log_τ)
...
function cal_pi_j_given_pi_i(ii::Int64, rank_i::Int64, concentration_parameter::Array{Array{Float64,1},1}, diff_ii::Array{Array{Float64,1},1}, sending_nodes_tt::Array{Int64,1}, sending_node_tt_coeff::Array{Int64,1}, receive_i::Int64, change_time_i::Array{Float64, 1}, t_i_s_list::Array{Array{Float64,N} where N,1}, log_β::Array{Float64,1}, pis_list::Array{Array{Array{Float64,1},1},1}, input_pi::Array{Float64, 1})
    ...
    for ii_index = 1:length(sending_nodes_tt)
        ...
        concentration_parameter[1] .+= exp(log_β[vv]).*pis_list[sending_nodes_tt[ii_index]][rank_ii]
        if sending_nodes_tt[ii_index] == ii
            diff_ii[1] = exp(log_β[vv]).*(input_pi.-pis_list[sending_nodes_tt[ii_index]][rank_ii])
        end
    end
...
function sample_K!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, M_seq ::Array{Float64, 1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, log_Λ ::Array{Float64,2}, t_i_s_list ::Array{Array{Float64,N} where N,1}, poisson_i_value ::Array{Float64, 1}, a_H::Array{Float64, 1})
    for ii = 1:hdbn.dataNum
        if rand()<0.5 # choose to propose a new change change point
            ...
            for ii_index = 1:length(sending_nodes_tt)
                ...
                concentration_parameter .+= exp(log_β[vv]).*pis_list[sending_nodes_tt[ii_index]][rank_ii]
            end
            ...
            for receive_i in receive_nodes_tt
                ...
                for rank_i = (start_rank:(end_rank-1))
                    ...
                    delta_current_ll, delta_proposal_ll = cal_pi_j_given_pi_i(ii, rank_i, concentration_parameter, diff_ii, sending_nodes_tt, sending_nodes_tt_coeff, receive_i, change_time_i, t_i_s_list, log_β, pis_list, pi_tt)
            ...
            for k1 = 1:hdbn.KK
                for k2 = 1:hdbn.KK
                    current_ll -= C_list[ii][current_index][k1]*integral_val_tt[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                    proposal_ll -= C_ii[k1]*integral_val_tt[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                end
            end
            for k1 = 1:hdbn.KK
            ...
            if length(t_i_s_list[ii])>0
                ...
                for receive_i in receive_nodes_tt
                    ...
                    if receive_i != ii
                        for rank_i = (start_rank:(end_rank-1))
                            ...
                            delta_current_ll, delta_proposal_ll = cal_pi_j_given_pi_i(ii, rank_i, concentration_parameter, diff_ii, sending_nodes_tt, sending_nodes_tt_coeff, receive_i, change_time_i, t_i_s_list, log_β, pis_list, input_pi)
                ...
                for k1 = 1:hdbn.KK
                    for k2 = 1:hdbn.KK
                        current_ll -= C_list[ii][current_index+1][k1]*integral_val_tt[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                        proposal_ll -= C_list[ii][current_index][k1]*integral_val_tt[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                    end
                end
                for k1 = 1:hdbn.KK
...
function sample_change_point!(hdbn ::HDBN, C_list ::Array{Array{Array{Float64,1},1},1}, pis_list ::Array{Array{Array{Float64,1},1},1}, b_ij ::Array{Int64,1}, u_ij ::Array{Int64,2}, log_Λ ::Array{Float64,2}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64, 1}, start_points::Float64, end_points::Float64)
    for ii = 1:hdbn.dataNum
        for jj = 1:length(t_i_s_list[ii])
            if (t_i_s_list[ii][jj]>start_points)&(t_i_s_list[ii][jj]<end_points)
    ...
    ########################
                        ...
                        for k1 = 1:hdbn.KK
                            for k2 = 1:hdbn.KK
                                current_ll -= C_list[ii][jj][k1]*(diff_integral[k2])*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                                proposal_ll -= C_list[ii][jj+1][k1]*(diff_integral[k2])*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
    ...
    ########################
                        ...
                        for k1 = 1:hdbn.KK
                            for k2 = 1:hdbn.KK
                                current_ll -= C_list[ii][jj+1][k1]*(diff_integral[k2])*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                                proposal_ll -= C_list[ii][jj][k1]*(diff_integral[k2])*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))/(a_H[1]^2)
                    ...
                    #         concentration_parameter .+= log_β[vv].*pis_list[sending_nodes_tt[ii_index]][rank_ii]
                    #     end
                    ...
                    #     for k1 = 1:hdbn.KK
                    #         for k2 = 1:hdbn.KK
                    #             current_ll -= C_list[ii][current_index][k1]*propose_integral[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))
                    #             current_ll -= C_list[ii][jj+1][k1]*current_integral[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))

                    #             proposal_ll -= C_ii[k1]*propose_integral[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))
                    #             proposal_ll -= C_list[ii][jj][k1]*current_integral[k2]*(exp(log_Λ[k1,k2])+exp(log_Λ[k2,k1]))
...
function ll_cal!(hdbn ::HDBN, b_ij ::Array{Int64,1}, log_α ::Array{Float64,1}, log_τ ::Array{Float64,1}, u_ij ::Array{Int64,2}, log_Λ ::Array{Float64,2}, C_list ::Array{Array{Array{Float64,1},1},1}, sender_receiver_num::Array{Int64, 2}, flattern_t_i_s_nodes ::Array{Int64,1}, flattern_t_i_s_nodes_rank ::Array{Int64,1}, t_i_s_list ::Array{Array{Float64,N} where N,1}, a_H::Array{Float64, 1})
    ll_val_part_1 = 0
    ll_val_part_2 = 0
    ll_val_part_3 = 0
    ll_val_part_4 = 0
    ...
    for tt = 1:hdbn.EdgeNum
        if b_ij[tt]==0
            ...
            C_list[hdbn.receive_node[tt]][sender_receiver_num[tt, 2]][u_ij[tt, 2]]*exp(log_Λ[u_ij[tt, 1], u_ij[tt, 2]])/(a_H[1]^2))
            ...
            ll_val_part_2 -= exp(log_τ[1])*(hdbn.eventtime[tt]-hdbn.eventtime[mutually_exciting_pair[tt][b_ij[tt]]])
            ll_val_part_2 += log_α[1]
        end
    end

    areas = cal_area(hdbn.KK, hdbn.dataNum, C_list, flattern_t_i_s_nodes, flattern_t_i_s_nodes_rank, t_i_s_list)

    ll_val_part_3 -= sum(areas.*exp.(log_Λ)./(a_H[1]^2))

    h_tau = 0
    for tt = 1:hdbn.EdgeNum
        h_tau += (1-exp(-exp(log_τ[1])*(hdbn.TT-hdbn.eventtime[tt])))
    end
    h_tau *= exp(log_τ[1])^(-1)

    ll_val_part_4 -= exp(log_α[1])*h_tau
...
function calculate_AUC(test_time_range, dataNum, KK, send_node_test, receive_node_test, eventtime_test, send_node, receive_node, eventtime, C_list_end, log_Λ, log_α, log_τ, a_H)
            ...
            for ii = 1:dataNum
                for jj = 1:dataNum
                    if ii!= jj
                        ...
                        for k1 = 1:KK
                            for k2 = 1:KK
                                rate += C_list_end[ii, k1]*exp(log_Λ[k1,k2])*C_list_end[jj, k2]/(a_H[1]^2)
                            end
                        end
                        rate *= (higher_v-lower_v)
                        for ij_time in historical_time_pair[jj][ii]
                            rate += exp(log_α[1])/exp(log_τ[1])*(exp(-exp(log_τ[1])*(lower_v-ij_time))-exp(-exp(log_τ[1])*(higher_v-ij_time)))
                        end
                        rate_val[ii, jj] = rate
                    end
                end
